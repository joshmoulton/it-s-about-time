-- Phase 4: Advanced Authentication & 2FA Implementation (Fixed)
...
-- Create 2FA secrets table for admin users (if not exists)
...
-- Create 2FA verification sessions table (if not exists)
...
-- Create admin security events table (if not exists)
...
CREATE INDEX IF NOT EXISTS idx_admin_2fa_secrets_email ON public.admin_2fa_secrets(admin_email);
CREATE INDEX IF NOT EXISTS idx_admin_2fa_sessions_token ON public.admin_2fa_sessions(session_token);
CREATE INDEX IF NOT EXISTS idx_admin_2fa_sessions_expires ON public.admin_2fa_sessions(expires_at);
CREATE INDEX IF NOT EXISTS idx_admin_security_events_email ON public.admin_security_events(admin_email);
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='admin_users' AND column_name='requires_2fa') THEN
        ALTER TABLE public.admin_users ADD COLUMN requires_2fa BOOLEAN DEFAULT true;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='admin_users' AND column_name='last_2fa_setup_at') THEN
        ALTER TABLE public.admin_users ADD COLUMN last_2fa_setup_at TIMESTAMP WITH TIME ZONE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='admin_users' AND column_name='failed_2fa_attempts') THEN
        ALTER TABLE public.admin_users ADD COLUMN failed_2fa_attempts INTEGER DEFAULT 0;
    END IF;
...
-- Apply triggers to new tables (if not exists)
DROP TRIGGER IF EXISTS update_admin_2fa_secrets_updated_at ON public.admin_2fa_secrets;
