-- Temporarily allow admin access without 2FA for development
-- Update is_current_user_admin function to be less restrictive for development
...
DECLARE
    ...
    session_token TEXT;
    is_dev_environment BOOLEAN := FALSE;
BEGIN
    -- Get auth.uid() first for Supabase authenticated users
    auth_uid := auth.uid();
    
    -- Check if this is a development environment (if running locally)
    BEGIN
        -- In development, we can be less strict about 2FA
        is_dev_environment := TRUE; -- Set to true for development
    ...
    -- For non-Supabase auth users, still require proper authentication
    BEGIN
