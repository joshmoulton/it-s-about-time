-- Fix highlight_topics table permissions and create sample data
...
-- Insert sample topics to get the chat highlights working
...
(
  ...
  'btc-price-discussion-' || extract(epoch from now())::text, 
...
(
  ...
  'options-trading-strategies-' || extract(epoch from now())::text, 
...
(
  ...
  'market-sentiment-analysis-' || extract(epoch from now())::text, 
...
(
  ...
  'crypto-altcoins-discussion-' || extract(epoch from now())::text, 
...
(
  ...
  'spy-trading-analysis-' || extract(epoch from now())::text, 
  ARRAY['SPY', 'S&P', 'index'], 
  15, 
  68, 
