-- Create enhanced live trading framework tables (fixed)
...
-- Create signals_overview VIEW for easy querying (fixed column reference)
CREATE OR REPLACE VIEW public.signals_overview AS
SELECT 
  id, ticker, direction, entry_type, entry_price, current_price, 
  stop_loss_price, targets, size_level, risk_score, confidence_score, 
  reasoning, analyst_id, status, created_at, updated_at,
  hit_targets, max_profit_pct, current_profit_pct, stopped_out,
  invalidation_type, invalidation_price, invalidation_tf, last_price
FROM public.live_trading_signals 
WHERE status = 'active';
