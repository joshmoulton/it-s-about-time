...
export const ModernAuthModal: React.FC<ModernAuthModalProps> = memo(({ open, onOpenChange, onExplicitClose }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={handleModalClose}>
      <DialogContent className="max-w-lg mx-auto p-0 bg-transparent border-none shadow-none [&>button]:hidden max-sm:fixed max-sm:inset-4 max-sm:top-8 max-sm:max-w-none max-sm:w-auto max-sm:h-auto max-sm:max-h-[calc(100vh-4rem)] max-sm:overflow-y-auto sm:fixed sm:left-[50%] sm:top-[50%] sm:translate-x-[-50%] sm:translate-y-[-50%] sm:max-w-lg sm:w-full modal-allowed">
