...
export function TopicSelector({ selectedTopic, onTopicChange }: TopicSelectorProps) {
  ...
  return (
    <Select value={selectedTopic || 'Money Glitch'} onValueChange={(value) => onTopicChange(value)}>
      <SelectTrigger className="w-40 sm:w-48 h-8 text-xs bg-slate-700 text-white border-slate-600 focus:border-blue-500">
        <SelectValue placeholder="Money Glitch" />
      </SelectTrigger>
      <SelectContent className="z-[99999] bg-slate-800 border-slate-700 text-white backdrop-blur-sm shadow-xl min-w-[160px] sm:min-w-[200px]" position="popper" sideOffset={8} align="start" avoidCollisions={true} collisionPadding={10}>
        {workingTopics.map((topic) => (
          <SelectItem 
            key={topic.id} 
            value={topic.custom_name} 
            className="text-xs hover:bg-slate-700 focus:bg-slate-700 px-3 py-2"
          >
            <div className="flex items-center gap-2 w-full">
              <MessageSquare className="h-3 w-3 flex-shrink-0" />
              <span className="text-xs">{topic.custom_name}</span>
            </div>
          </SelectItem>
        ))}
      </SelectContent>
